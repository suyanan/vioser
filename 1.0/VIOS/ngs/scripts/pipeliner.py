...
def process_pipeliner_pe(genomeHostName,ntVirusesFamilyName):
    ...
    for i in range(len(sample_list)):
        ...
        os.system('java -jar $TRIMMOMATIC_HOME/trimmomatic.jar PE -threads %d -trimlog %s %s %s %s %s %s %s ILLUMINACLIP:$TRIMMOMATIC_HOME/adapters/TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36'
...
def process_pipeliner_se(genomeHostName,ntVirusesFamilyName):
    ...
    for i in range(len(sample_se_list)):
        ...
        os.system('java -jar $TRIMMOMATIC_HOME/trimmomatic-0.36.jar SE -threads %d -trimlog %s %s %s ILLUMINACLIP:$TRIMMOMATIC_HOME/adapters/TruSeq3-SE.fa:2:30:10 LEADING:%d TRAILING:%d SLIDINGWINDOW:4:15 MINLEN:%d'
